...
namespace Scene.MainGameScene.Battle
{
    // 몬스터 머리 위 체력 표시.
    //
    // UGUI 캔버스 대신 SpriteRenderer 두 장으로 만듭니다.
    // 한 화면에 수십 마리가 깔리는데 그만큼 월드 캔버스를 두면
    // 캔버스 리빌드 비용이 마리 수에 그대로 비례해 붙습니다.
    public class MonsterHealthBarView : MonoBehaviour
    {
        [SerializeField]
        [Tooltip("바탕(닳은 부분)")]
        ...
        private void CacheUnitSize()
        {
            ...
            // 같은 크기면 어두운 배경에서 채움에 완전히 가려 보이지 않습니다.
