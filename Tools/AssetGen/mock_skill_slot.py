...
2. **두 자리가 나온다.** 상한은 **20**이다. 도달 레벨 20이라 레벨업이 19회인데,
   `SkillState.Level => 1 + _totalUpgradeCount`이고 `_totalUpgradeCount`는
   `ApplyUpgrade`에서만 오른다. 그래서 19회를 전부 한 스킬 강화에 쓰면 20이다.

   레벨업을 안 쓰는 경로가 둘 있어서 상한이 여기까지 온다.
   - **시작 스킬은 공짜다.** `MainGameController.GrantStartSkill()`이 3택을 안
     거치고 `_inventory.Add`로 직행한다.
   - **강화스킬은 표시 레벨을 안 올린다.** `SetMastery`는 `Mastery`만 바꾼다.
     레벨업 한 번을 쓰지만 숫자는 그대로다.

   리롤도 카드를 다시 뽑을 뿐 고르는 횟수를 늘리지 않는다.

   **두 번 틀렸다.** 처음엔 도달 17 시절 값인 14로 잡았고, 도달 20으로 고치면서는
   "스킬 3종 획득에 3회를 쓴다"를 전제해 17이라 했다. **시작 스킬이 공짜라는 걸
   안 봤다.** 시안 결론은 세 번 다 같다 - 14도 17도 20도 두 자리라 판 크기와
   폰트가 그대로다. 세 자리는 도달 레벨 30 근처에서 나온다.
...
def main():
    ...
    rows = (
        ("A", ["Lv.1", "Lv.7", "Lv.13", "Lv.20", None], 100, 4),
        ("B", ["1", "7", "13", "20", None], 62, 6),
    )
    cols = 5
    ...
    print("두 안 모두 두 자리(상한 20)까지 안 깨진다. 마지막 칸은 빈 슬롯.")
