...
2. **두 자리가 나온다.** 상한은 14다 - 최종 레벨 17이라 레벨업이 16회, 스킬 3종
   획득에 3회를 쓰면 한 스킬에 몰 수 있는 강화가 최대 13회, 거기에 획득 1을 더한다.
   리롤은 카드를 다시 뽑을 뿐 고르는 횟수를 늘리지 않으므로 여기 안 들어간다.
   웨이브 20 고정이 최종 레벨을 고정하는 한 세 자리는 안 나온다.
...
def main():
    ...
    rows = (
        ("A", ["Lv.1", "Lv.5", "Lv.9", "Lv.14", None], 100, 4),
        ("B", ["1", "5", "9", "14", None], 62, 6),
    )
    cols = 5
    ...
    print("두 안 모두 두 자리(상한 14)까지 안 깨진다. 마지막 칸은 빈 슬롯.")
