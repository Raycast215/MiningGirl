...
각 함수 docstring의 이름은 `MonsterDataTable`의 표시명(Name)과 같게 맞춘다.
그림과 이름이 갈라지면 도감이나 미리보기가 생기는 순간 모순으로 드러난다.
2026-08-27에 001~003이 갈라져 있어 기획이 이름을 그림 쪽으로 맞췄다
(광충 -> 광석 슬라임, 날벌레 -> 동굴 박쥐, 바위게 -> 바위 거미).

파일 이름(`Monster_003_Spider.png`)은 안 맞춰도 된다 - 어드레서블과 프리팹이
물고 있는 자산 식별자이고, 표시명과는 다른 것이다.
"""
...
def spider():
    """Monster_003 : 바위 거미 - 다리 8개

    몸통이 청회색이라 바닥의 밝은 얼룩과 정확히 겹치는 자리가 다섯 스테이지
    전부에 있다(최악 대비 1.00~1.06). 광재와 같은 두 톤 테두리를 준다 -
    위는 밝고 아래는 어두워서, 바닥이 몸보다 밝든 어둡든 한쪽 모서리가 남는다.
    """
    g = blank()
    ...
    rim(g, (146, 152, 186, 255), (24, 26, 40, 255))
    return g
