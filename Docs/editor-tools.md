...
- **측정 전에 저장 파일을 지우십시오.** 진입 우선순위가 `지정 진입 > 저장 >
  ...
  **`SessionState` 함정보다 나쁩니다.** 그쪽은 엉뚱한 스테이지가 열려 눈에 띄는데,
  **이건 맞는 스테이지가 열리고 결과만 거짓입니다.** 그래서 판 끝 로그가
  `[저장 복원된 판 - 측정에 쓰지 마십시오]`를 찍습니다(`MainGameController.DebugIsRestored`).
  절차보다 이쪽을 믿으십시오 — **절차는 잊히고 로그는 남습니다**
